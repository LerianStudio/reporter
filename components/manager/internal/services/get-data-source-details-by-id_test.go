...
func Test_getBaseCollectionName(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			collectionName: "some_table_name_org456",
			expected:       "some_table_name",
		},
		{
			name:           "collection with single underscore",
			collectionName: "holders_",
			expected:       "holders",
		},
		{
...
func Test_shouldIncludeFieldForPluginCRM(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "search field should be included",
			fieldName:      "search",
			collectionName: "holders",
			expected:       true,
		},
		{
			name:           "encrypted field should be excluded",
			fieldName:      "document",
			collectionName: "holders",
			expected:       false,
		},
		{
			name:           "nested encrypted field should be excluded",
			fieldName:      "natural_person.mother_name",
			collectionName: "holders",
			expected:       false,
		},
		{
			name:           "regular field in holders should be included",
			fieldName:      "_id",
			collectionName: "holders",
			expected:       true,
		},
		{
			name:           "regular field in aliases should be included",
			fieldName:      "account_id",
			collectionName: "aliases",
			expected:       true,
		},
		{
			name:           "regular field in other collection should be included",
...
func Test_getExpandedFieldsForPluginCRM(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			collectionName: "holders",
			expectNil:      false,
			expectContains: []string{"_id", "external_id", "type", "addresses", "search.document"},
		},
		{
			name:           "aliases collection returns expanded fields",
			collectionName: "aliases",
			expectNil:      false,
			expectContains: []string{"_id", "account_id", "holder_id", "search.document"},
		},
		{
		...
		{
			name:           "empty collection returns nil",
			collectionName: "",
			expectNil:      true,
			expectContains: nil,
		},
...
func Test_getFieldsForPluginCRM(t *testing.T) {
	uc := &UseCase{}
	...
	tests := []struct {
		...
		{
			name: "holders collection uses expanded fields",
			collection: mongodb.CollectionSchema{
				CollectionName: "holders_org123",
				Fields: []mongodb.FieldInformation{
					{Name: "_id"},
					{Name: "external_id"},
					{Name: "document"}, // encrypted - should not appear (using expanded fields instead)
				},
			},
			expectContains: []string{"_id", "external_id", "type"},
		},
		{
			name: "aliases collection uses expanded fields",
			collection: mongodb.CollectionSchema{
				CollectionName: "aliases_org123",
				Fields: []mongodb.FieldInformation{
					{Name: "_id"},
					{Name: "account_id"},
				},
			},
			expectContains: []string{"_id", "account_id", "holder_id"},
		},
		{
			name: "unknown collection filters by shouldInclude",
			collection: mongodb.CollectionSchema{
				CollectionName: "other_collection_org123",
				Fields: []mongodb.FieldInformation{
					{Name: "_id"},
					{Name: "some_field"},
					{Name: "document"}, // encrypted - should be filtered
				},
			},
			expectContains: []string{"_id", "some_field"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := uc.getFieldsForPluginCRM(tt.collection)
			assert.NotNil(t, result)
			for _, expected := range tt.expectContains {
				assert.Contains(t, result, expected)
			}
...
func Test_getDisplayNameForCollection(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "plugin_crm removes org suffix",
			collectionName: "holders_org123",
			dataSourceID:   "plugin_crm",
			expected:       "holders",
		},
		{
			name:           "non plugin_crm returns original",
			collectionName: "holders_org123",
			dataSourceID:   "other_datasource",
			expected:       "holders_org123",
		},
		{
			name:           "plugin_crm with simple name",
			collectionName: "users",
			dataSourceID:   "plugin_crm",
			expected:       "users",
		},
