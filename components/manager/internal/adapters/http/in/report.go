...
func (rh *ReportHandler) GetDownloadReport(c *fiber.Ctx) error {
	...
	if strings.ToLower(templateModel.OutputFormat) == "pdf" {
		...
		if err != nil {
			libOpentelemetry.HandleSpanError(&span, "Failed to create temporary file", err)
			logger.Errorf("Failed to create temporary file for PDF generation: %v", err)
		...
		err = rh.Service.PdfPool.Submit(string(fileBytes), tmpFileName)
		if err != nil {
		...
		// Read the generated PDF file - tmpFileName is safe as it comes from os.CreateTemp
