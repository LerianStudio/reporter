...
func (repo *SimpleRepository) Get(ctx context.Context, objectName string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get template from SeaweedFS: %w, bucket: %s, object: %s", err, repo.bucket, objectName)
	}
...
func (repo *SimpleRepository) Put(ctx context.Context, objectName string, contentType string, data []byte) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to put template to SeaweedFS: %w", err)
	}
