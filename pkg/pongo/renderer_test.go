...
Calculation: {% calc (100 + 200) * 1.2 %}`)
	...
	assert.Contains(t, out, "Calculation: 360") // (100 + 200) * 1.2 = 360
}
...
func TestRender_CalcTag(t *testing.T) {
	...
	out, err := r.RenderFromBytes(context.Background(), tpl, data, logger)
	assert.NoError(t, err)
	assert.Contains(t, out, "300") // 100 + 200 = 300
}
...
{% endfor %}`)
	...
	data := map[string]map[string][]map[string]any{
		"midaz_transaction": {
			"balance": {
				{"alias": "Account1", "available": ""},      // Empty string
				{"alias": "Account2", "available": "100.5"}, // Valid number
			},
		},
	}
	...
	assert.Contains(t, out, "Sum: 1.2") // 0 + 1.2 = 1.2
	assert.Contains(t, out, "Alias: Account2")
	assert.Contains(t, out, "Balance: 100.5")
	assert.Contains(t, out, "Sum: 101.7") // 100.5 + 1.2 = 101.7
}
...
{% endfor %}`)
	...
	data := map[string]map[string][]map[string]any{
		"midaz_transaction": {
			"balance": {
				{"alias": "Account1", "available": "0", "on_hold": "3.50"},    // 0 + 1.2 = 1.2, (0+1.2)*3.50-0/2 = 4.2
				{"alias": "Account2", "available": "100.5", "on_hold": "2.0"}, // 100.5 + 1.2 = 101.7, (100.5+1.2)*2.0-100.5/2 = 203.4-50.25 = 153.15
			},
		},
	}
...
Sum: {% calc (midaz_transaction.balance.3.available + 1.2) ** 2 %}`)
	...
	data := map[string]map[string][]map[string]any{
		"midaz_transaction": {
			"balance": {
				{"alias": "Account1", "available": "3"},  // 3 ** 2 = 9
				{"alias": "Account2", "available": "4"},  // 4 ** 2 = 16
				{"alias": "Account3", "available": "5"},  // 5 ** 2 = 25
				{"alias": "Account4", "available": "10"}, // (10 + 1.2) ** 2 = 11.2 ** 2 = 125.44
			},
		},
	}
	...
	// Check for the power result with floating-point precision tolerance
...
Sum: {% calc (midaz_transaction.balance.3.available + 1.2) ** 2 %}`)
	...
	assert.Contains(t, out, "Alias: Account2")
	assert.Contains(t, out, "Alias: Account3")
	// The last calc should handle the missing index gracefully
	assert.Contains(t, out, "Sum: 1.44") // (0 + 1.2) ** 2 = 1.44 (when index 3 doesn't exist)
}
...
Power Fractional: {% calc 2.5 ** 0.5 %}`)
	...
	out, err := r.RenderFromBytes(context.Background(), tpl, data, logger)
	assert.NoError(t, err)
	assert.Contains(t, out, "Power Small: 0.001")             // 0.1^3 = 0.001
	assert.Contains(t, out, "Power Large: 1000000")           // 1000^2 = 1000000
	assert.Contains(t, out, "Power Fractional: 1.5811388301") // 2.5^0.5 = sqrt(2.5) ≈ 1.5811388301
}
